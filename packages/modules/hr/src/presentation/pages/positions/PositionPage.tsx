...
import { positionApi, departmentApi } from "../../../application/hr.api";
...
const LEVELS = [
  ...
  { value: "mid", label: "IntermÃ©diaire" },
...
export default function PositionPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SuccÃ¨s!", `Le poste a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"}.`, "success"); qc.invalidateQueries({ queryKey: ["positions"] }); closeDialog(); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SupprimÃ©!", "Poste supprimÃ©.", "success"); qc.invalidateQueries({ queryKey: ["positions"] }); },
  ...
  const columns = [
    ...
    { id: "code", label: "Code" },
    { id: "department", label: "DÃ©partement", render: (r: PosItem) => r.department?.name || "â€”" },
    { id: "level", label: "Niveau", render: (r: PosItem) => LEVELS.find((l) => l.value === r.level)?.label || r.level || "â€”" },
    { id: "salary", label: "Fourchette salariale", render: (r: PosItem) => r.minSalary || r.maxSalary ? `${r.minSalary || "?"} â€“ ${r.maxSalary || "?"}` : "â€”" },
                ...
                <form onSubmit={formik.handleSubmit}>
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div><Label>Code</Label><Input name="code" value={formik.values.code} onChange={formik.handleChange} />{formik.touched.code && formik.errors.code && <p className="text-destructive text-xs mt-1">{formik.errors.code}</p>}</div>
                    </div>
                    <div>
