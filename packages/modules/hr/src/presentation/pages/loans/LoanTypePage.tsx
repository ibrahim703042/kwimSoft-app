...
import { loanTypeApi } from "../../../application/hr.api";
...
const schema = Yup.object({
  ...
  interestRate: Yup.number().min(0, "Doit Ãªtre positif"),
  maxAmount: Yup.number().min(0, "Doit Ãªtre positif"),
  minAmount: Yup.number().min(0, "Doit Ãªtre positif"),
...
export default function LoanTypePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "SuccÃ¨s!",
        `Le type de prÃªt a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"} avec succÃ¨s.`,
        "success"
      );
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SupprimÃ©!", "Type de prÃªt supprimÃ©.", "success");
      qc.invalidateQueries({ queryKey: ["loan-types"] });
    },
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Montant Max",
      cell: ({ row }: any) =>
        row.original.maxAmount?.toLocaleString() || "â€”",
    },
    {
      accessorKey: "maxTenureMonths",
      header: "DurÃ©e Max (mois)",
      cell: ({ row }: any) => row.original.maxTenureMonths || "â€”",
    },
    {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold">Types de PrÃªt</h2>
          <p className="text-sm text-muted-foreground">
            GÃ©rer les types de prÃªts disponibles
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
            <form onSubmit={formik.handleSubmit}>
              <DialogHeader>
                <DialogTitle>
                  {editing ? "Modifier le type de prÃªt" : "Nouveau type de prÃªt"}
                </DialogTitle>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>DurÃ©e minimum (mois)</Label>
                    <Input
                  ...
                  <div>
                    <Label>DurÃ©e maximum (mois)</Label>
                    <Input
                ...
                <div className="flex gap-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor="requiresApproval">NÃ©cessite approbation</Label>
                  </div>
                </div>
              </div>
