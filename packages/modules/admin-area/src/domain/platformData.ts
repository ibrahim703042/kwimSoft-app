...
import { APP_CONFIG, isProduction } from "@kwim/config";
...
export function getRegisteredModuleNames(): string[] {
  return getRegisteredModules().map((m) => m.name);
}
...
export const DASHBOARD_QUICK_ACTIONS: QuickAction[] = [
  ...
  { id: "product", label: "Products", description: "Catalog, categories, pricing", path: "/products" },
