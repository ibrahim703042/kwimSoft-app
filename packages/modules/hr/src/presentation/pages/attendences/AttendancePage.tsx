...
import { attendanceApi, employeeApi } from "../../../application/hr.api";
...
const STATUS_OPTIONS = [
  ...
  { value: "half_day", label: "Demi-journÃ©e" },
  { value: "remote", label: "TÃ©lÃ©travail" },
];

const ATTENDANCE_TYPE_OPTIONS = [
  { value: "manual", label: "Manuel" },
  { value: "biometric", label: "BiomÃ©trique" },
...
export default function AttendancePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SuccÃ¨s!", `Le pointage a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"}.`, "success");
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SupprimÃ©!", "Pointage supprimÃ©.", "success");
      qc.invalidateQueries({ queryKey: ["attendances"] });
    },
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Date",
      cell: ({ row }: any) =>
        row.original.date ? new Date(row.original.date).toLocaleDateString() : "â€”",
    },
    {
      accessorKey: "checkInTime",
      header: "ArrivÃ©e",
      cell: ({ row }: any) => row.original.checkInTime || "â€”",
    },
    {
      accessorKey: "checkOutTime",
      header: "DÃ©part",
      cell: ({ row }: any) => row.original.checkOutTime || "â€”",
    },
    {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            GÃ©rer les pointages des employÃ©s
          </p>
        </div>
        <Button
