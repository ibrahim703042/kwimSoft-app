/**
 * HR Dashboard â€” Stats and overview from existing HR APIs
 *
...
async function fetchHrDashboardData() {
  ...
  const attendanceToday = attendances.filter(
    (a: any) => a.date?.toString().slice(0, 10) === today
  ).length;
  ...
  return {
    ...
    leaveByStatus: [
      ...
      { name: "ApprouvÃ©", value: leaveApproved },
      { name: "RejetÃ©", value: leaves.filter((l: any) => l.status === "rejected").length },
      { name: "AnnulÃ©", value: leaves.filter((l: any) => l.status === "cancelled").length },
    ].filter((s) => s.value > 0),
    payrollByStatus: [
      { name: "Brouillon", value: payrollDraft },
      { name: "ConfirmÃ©", value: payrolls.filter((p: any) => p.status === "confirmed").length },
      { name: "PayÃ©", value: payrollPaid },
      { name: "AnnulÃ©", value: payrolls.filter((p: any) => p.status === "cancelled").length },
    ].filter((s) => s.value > 0),
  };
}
