...
import { recruitmentApi, departmentApi, positionApi } from "../../../application/hr.api";
...
export default function RecruitmentPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SuccÃ¨s!", `L'offre a Ã©tÃ© ${editing ? "modifiÃ©e" : "crÃ©Ã©e"}.`, "success"); qc.invalidateQueries({ queryKey: ["recruitment"] }); closeDialog(); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SupprimÃ©!", "Offre supprimÃ©e.", "success"); qc.invalidateQueries({ queryKey: ["recruitment"] }); },
  ...
  const columns = [
    ...
    { id: "department", label: "DÃ©partement", render: (r: RecruitItem) => r.department?.name || "â€”" },
    { id: "position", label: "Poste", render: (r: RecruitItem) => r.position?.title || "â€”" },
    { id: "applicationCount", label: "Candidatures", render: (r: RecruitItem) => <span>{r.applicationCount || 0}</span> },
    { id: "closeDate", label: "Date limite", render: (r: RecruitItem) => r.closeDate ? new Date(r.closeDate).toLocaleDateString() : "â€”" },
