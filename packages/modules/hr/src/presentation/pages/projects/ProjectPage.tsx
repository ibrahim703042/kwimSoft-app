...
import { projectApi, clientApi, employeeApi } from "../../../application/hr.api";
...
export default function ProjectPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "SuccÃ¨s!",
        `Le projet a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"} avec succÃ¨s.`,
        "success"
      );
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SupprimÃ©!", "Projet supprimÃ©.", "success");
      qc.invalidateQueries({ queryKey: ["projects"] });
    },
  ...
  const columns = [
    ...
    { accessorKey: "code", header: "Code" },
    {
      accessorKey: "client",
      header: "Client",
      cell: ({ row }: any) => row.original.client?.name || "â€”",
    },
    {
      accessorKey: "manager",
      header: "Responsable",
      cell: ({ row }: any) => {
        ...
        return m ? `${m.firstName || ""} ${m.lastName || ""}`.trim() : "â€”";
      },
    },
    {
      accessorKey: "status",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            GÃ©rer les projets de l'entreprise
          </p>
        </div>
