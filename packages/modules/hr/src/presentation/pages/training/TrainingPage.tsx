...
import { trainingApi } from "../../../application/hr.api";
...
const schema = Yup.object({
  ...
  trainer: Yup.string(),
  startDate: Yup.string().required("La date de dÃ©but est requise"),
  endDate: Yup.string(),
  location: Yup.string(),
...
export default function TrainingPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SuccÃ¨s!", `La formation a Ã©tÃ© ${editing ? "modifiÃ©e" : "crÃ©Ã©e"}.`, "success"); qc.invalidateQueries({ queryKey: ["trainings"] }); closeDialog(); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SupprimÃ©!", "Formation supprimÃ©e.", "success"); qc.invalidateQueries({ queryKey: ["trainings"] }); },
  ...
  const columns = [
    ...
    { id: "type", label: "Type", render: (r: TrainingItem) => TRAINING_TYPES.find((t) => t.value === r.type)?.label || r.type || "â€”" },
    { id: "trainer", label: "Formateur" },
    { id: "startDate", label: "DÃ©but", render: (r: TrainingItem) => r.startDate ? new Date(r.startDate).toLocaleDateString() : "â€”" },
              ...
              <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                <form onSubmit={formik.handleSubmit}>
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div><Label>DÃ©but</Label><Input name="startDate" type="date" value={formik.values.startDate} onChange={formik.handleChange} />{formik.touched.startDate && formik.errors.startDate && <p className="text-destructive text-xs mt-1">{formik.errors.startDate}</p>}</div>
