...
const schema = Yup.object({
  amount: Yup.number().min(0.01, "Le montant doit Ãªtre positif").required("Le montant est requis"),
...
export default function LoanPaymentsPage() {
  ...
  const recordPaymentMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SuccÃ¨s!", "Le paiement a Ã©tÃ© enregistrÃ©.", "success");
  ...
  const statusConfig = {
    paid: { color: "bg-green-100 text-green-800", icon: CheckCircle, label: "PayÃ©" },
    overdue: { color: "bg-red-100 text-red-800", icon: AlertTriangle, label: "En retard" },
    upcoming: { color: "bg-yellow-100 text-yellow-800", icon: Clock, label: "Ã€ venir" },
    scheduled: { color: "bg-gray-100 text-gray-800", icon: Calendar, label: "PlanifiÃ©" },
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Reste Ã  payer</p>
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        <div className="flex justify-between text-xs text-muted-foreground mt-2">
          <span>{paidPayments.length} paiements effectuÃ©s</span>
      ...
      <div className="rounded-lg border bg-card p-6">
        <h3 className="flex items-center gap-2 text-lg font-semibold mb-4">
          <Receipt className="h-5 w-5" /> Ã‰chÃ©ancier des paiements
        </h3>
        <div>
          {repayments.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              <Calendar className="mx-auto h-12 w-12 mb-4 opacity-50" />
              <p>Aucun Ã©chÃ©ancier gÃ©nÃ©rÃ© pour ce prÃªt.</p>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {payment.notes && <span className="ml-2">â€¢ {payment.notes}</span>}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {payment.isPaid ? (
                        <div className="text-right">
                          <p className="text-sm font-medium text-green-600">
                            PayÃ© le {payment.paidDate ? new Date(payment.paidDate).toLocaleDateString("fr-FR") : "â€”"}
                          </p>
                          {payment.transactionReference && (
                            <p className="text-xs text-muted-foreground">RÃ©f: {payment.transactionReference}</p>
                          )}
                        </div>
                      ) : (
                        loan.status === "active" && (
                          <Button size="sm" onClick={() => openScheduledPayment(idx, payment)}>
                            <CheckCircle className="h-4 w-4 mr-1" /> Marquer payÃ©
                          </Button>
                        )
                      )}
                    </div>
      ...
      <Dialog open={paymentOpen} onOpenChange={setPaymentOpen}>
        <DialogContent className="sm:max-w-md">
          <form onSubmit={formik.handleSubmit}>
            <DialogHeader>
              <DialogTitle>
                {selectedPaymentIndex !== null ? "Marquer comme payÃ©" : "Enregistrer un paiement"}
              </DialogTitle>
            </DialogHeader>
