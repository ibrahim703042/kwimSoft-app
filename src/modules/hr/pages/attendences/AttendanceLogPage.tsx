 ...
 *
 * Displays SN, Employee Name, Attendance Type, Identifier (MAC/IP/device), Date.
 */
...
export default function AttendanceLogPage() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (department) {
      list = list.filter(
        (r) => r.employee?.department?._id === department || r.employee?.department === department
      );
    }
    if (logTab === "manual") {
      ...
      list = list.filter(
        (r) =>
          r.attendanceType &&
          ["biometric", "wifi", "qr", "ip"].includes(r.attendanceType)
      );
    }
    return list;
      ...
      <div className="bg-white dark:bg-[#1a1f3b] rounded-xl border border-border/40 p-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="min-w-[180px]">
            <label className="text-xs text-muted-foreground block mb-1">Select Branch</label>
          ...
          <div className="min-w-[180px]">
            <label className="text-xs text-muted-foreground block mb-1">Select Department</label>
          ...
          <Button size="sm" className="bg-[#0F123F] hover:bg-[#0F123F]/90">
            Filter
          </Button>
        ...
        <div className="border-b flex mt-2">
          <button
            ...
            className={cn(
              ...
              logTab === "manual"
                ? "border-b-2 border-[#0F123F] text-[#0F123F] bg-muted/30"
              ...
              {isLoading ? (
                ...
                filteredLogs.map((row: any, index: number) => (
                  <TableRow key={row._id || index}>
                    ...
                    <TableCell>
                      {ATTENDANCE_TYPE_LABELS[row.attendanceType] || row.attendanceType || "manual"}
                    </TableCell>
