...
import { checkUrlReachable, type HealthResult } from "../../domain/lib/healthCheck";
...
export default function SystemStatusPage() {
  const [statuses, setStatuses] = useState<Record<ServiceKey, ServiceStatus>>(() =>
    services.reduce<Record<ServiceKey, ServiceStatus>>((acc, s) => {
      acc[s.key] = { state: "idle" };
      return acc;
    ...
    setStatuses((prev) => {
      ...
      results.forEach(({ key, result }) => {
        if (result.status === "connected") {
          next[key] = { state: "connected", statusCode: result.statusCode };
        } else {
          next[key] = { state: "failed", error: result.error };
        }
      });
      return next;
    });
