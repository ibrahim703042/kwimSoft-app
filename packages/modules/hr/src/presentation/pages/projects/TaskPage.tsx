...
import { taskApi, projectApi, employeeApi } from "../../../application/hr.api";
...
const statuses = [
  ...
  { value: "in_review", label: "En rÃ©vision" },
...
export default function TaskPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "SuccÃ¨s!",
        `La tÃ¢che a Ã©tÃ© ${editing ? "modifiÃ©e" : "crÃ©Ã©e"} avec succÃ¨s.`,
        "success"
      );
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SupprimÃ©!", "TÃ¢che supprimÃ©e.", "success");
      qc.invalidateQueries({ queryKey: ["tasks"] });
    },
  ...
  const columns = [
    ...
    },
    {
      accessorKey: "project",
      header: "Projet",
      cell: ({ row }: any) => row.original.project?.name || "â€”",
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return a ? `${a.firstName || ""} ${a.lastName || ""}`.trim() : "â€”";
      },
    },
    {
      accessorKey: "status",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            GÃ©rer les tÃ¢ches des projets
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            ...
            >
              <Plus className="h-4 w-4 mr-2" /> Nouvelle tÃ¢che
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
            <form onSubmit={formik.handleSubmit}>
              <DialogHeader>
                <DialogTitle>
                  {editing ? "Modifier la tÃ¢che" : "Nouvelle tÃ¢che"}
                </DialogTitle>
              </DialogHeader>
