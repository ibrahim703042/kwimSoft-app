...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@kwim/shared-ui/dialog";
import { Label } from "@kwim/shared-ui/label";
import { Input } from "@kwim/shared-ui/input";
import { Textarea } from "@kwim/shared-ui/textarea";
import { Select, SelectContent, SelectGroup, SelectItem, SelectTrigger, SelectValue } from "@kwim/shared-ui/select";
import { useFormik } from "formik";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import Swal from "sweetalert2";
import * as Yup from "yup";
import { expenseApi, employeeApi } from "../../../application/hr.api";
...
const EXPENSE_TYPES = [
  ...
  { value: "equipment", label: "Ã‰quipement" },
  { value: "communication", label: "Communication" },
  { value: "training", label: "Formation" },
  { value: "medical", label: "MÃ©dical" },
...
const schema = Yup.object({
  ...
  employee: Yup.string().required("L'employÃ© est requis"),
...
export default function ExpensePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SuccÃ¨s!", `La dÃ©pense a Ã©tÃ© ${editing ? "modifiÃ©e" : "crÃ©Ã©e"}.`, "success"); qc.invalidateQueries({ queryKey: ["expenses"] }); closeDialog(); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SupprimÃ©!", "DÃ©pense supprimÃ©e.", "success"); qc.invalidateQueries({ queryKey: ["expenses"] }); },
  ...
  const empName = (e?: ExpenseItem["employee"]) => e ? (e.fullName || `${e.firstName || ""} ${e.lastName || ""}`.trim()) : "â€”";
  const handleDelete = (id: string) => {
    Swal.fire({ title: "ÃŠtes-vous sÃ»r?", text: "Cette action est irrÃ©versible!", icon: "warning", showCancelButton: true, confirmButtonColor: "#d33", cancelButtonColor: "#3085d6", confirmButtonText: "Oui, supprimer!", cancelButtonText: "Annuler" })
  ...
  const columns = [
    { id: "reference", label: "RÃ©f." },
    { id: "employee", label: "EmployÃ©", render: (r: ExpenseItem) => empName(r.employee) },
    { id: "type", label: "Type", render: (r: ExpenseItem) => EXPENSE_TYPES.find((t) => t.value === r.type)?.label || r.type || "â€”" },
    { id: "amount", label: "Montant", render: (r: ExpenseItem) => `${(r.amount || 0).toLocaleString()} ${r.currency || "CDF"}` },
    { id: "date", label: "Date", render: (r: ExpenseItem) => r.date ? new Date(r.date).toLocaleDateString() : "â€”" },
    ...
    <div>
          ...
          <div className="flex justify-end mb-3">
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="bg-[#0F123F] py-2 px-2 text-[0.8rem]" size="sm" onClick={() => setEditing(null)}><Plus color="white" /> Nouvelle dÃ©pense</Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                <form onSubmit={formik.handleSubmit}>
                  <DialogHeader><DialogTitle>{editing ? "Modifier la dÃ©pense" : "Nouvelle dÃ©pense"}</DialogTitle></DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div><Label>RÃ©fÃ©rence</Label><Input name="reference" value={formik.values.reference} onChange={formik.handleChange} /></div>
      ...
      <Dialog open={viewOpen} onOpenChange={setViewOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>DÃ©tails de la dÃ©pense</DialogTitle></DialogHeader>
          {viewing && (
            <div className="grid grid-cols-2 gap-3 text-sm py-2">
              <div><span className="font-medium text-muted-foreground">RÃ©f.:</span> {viewing.reference || "â€”"}</div>
              <div><span className="font-medium text-muted-foreground">EmployÃ©:</span> {empName(viewing.employee)}</div>
              <div><span className="font-medium text-muted-foreground">Type:</span> {EXPENSE_TYPES.find((t) => t.value === viewing.type)?.label || viewing.type || "â€”"}</div>
              ...
              <div><span className="font-medium text-muted-foreground">Date:</span> {viewing.date ? new Date(viewing.date).toLocaleDateString() : "â€”"}</div>
              <div><span className="font-medium text-muted-foreground">Statut:</span> {statusBadge(viewing.status)}</div>
              <div className="col-span-2"><span className="font-medium text-muted-foreground">Description:</span> {viewing.description || "â€”"}</div>
              {viewing.approvedBy && <div className="col-span-2"><span className="font-medium text-muted-foreground">ApprouvÃ© par:</span> {viewing.approvedBy.fullName}</div>}
            </div>
          )}
        </DialogContent>
      </Dialog>
