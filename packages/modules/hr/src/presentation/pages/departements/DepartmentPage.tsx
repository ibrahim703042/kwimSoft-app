...
} from "@kwim/shared-ui/select";
import { useFormik } from "formik";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import Swal from "sweetalert2";
import * as Yup from "yup";
import { departmentApi, employeeApi } from "../../../application/hr.api";
...
export default function DepartmentPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SuccÃ¨s!", `Le dÃ©partement a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"} avec succÃ¨s.`, "success");
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { Swal.fire("SupprimÃ©!", "DÃ©partement supprimÃ©.", "success"); qc.invalidateQueries({ queryKey: ["departments"] }); },
  ...
  const columns = [
    ...
    { id: "manager", label: "Responsable", render: (r: DeptItem) => {
      if (!r.manager) return "â€”";
      return r.manager.fullName || `${r.manager.firstName || ""} ${r.manager.lastName || ""}`.trim() || "â€”";
    }},
    { id: "employeeCount", label: "EmployÃ©s", render: (r: DeptItem) => <span>{r.employeeCount || 0}</span> },
    { id: "parentDepartment", label: "DÃ©partement parent", render: (r: DeptItem) => r.parentDepartment?.name || "â€”" },
  ...
  return (
    <div>
      <div className="bg-card py-2 px-3 h-full border rounded-md">
        <div><div className="bg-slate-100 p-1 rounded px-2 py-1"><p className="bg-[#0F123F] inline-block text-white px-3 py-1 rounded text-[0.7rem]">DÃ©partements</p></div></div>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="bg-[#0F123F] py-2 px-2 text-[0.8rem]" size="sm" onClick={() => setEditing(null)}>
                  <Plus color="white" /> Ajouter un dÃ©partement
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                <form onSubmit={formik.handleSubmit}>
                  <DialogHeader><DialogTitle>{editing ? "Modifier le dÃ©partement" : "Nouveau dÃ©partement"}</DialogTitle></DialogHeader>
                  <div className="grid gap-4 py-4">
                    ...
                    <div>
                      <Label>DÃ©partement parent</Label>
