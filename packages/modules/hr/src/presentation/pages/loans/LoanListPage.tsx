...
import { loanApi, loanTypeApi, employeeApi } from "../../../application/hr.api";
...
const schema = Yup.object({
  ...
  loanType: Yup.string(),
  principalAmount: Yup.number().min(1, "Le montant doit Ãªtre positif").required("Le montant est requis"),
...
export default function LoanListPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire(
        "SuccÃ¨s!",
        `Le prÃªt a Ã©tÃ© ${editing ? "modifiÃ©" : "crÃ©Ã©"} avec succÃ¨s.`,
        "success"
      );
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      Swal.fire("SupprimÃ©!", "PrÃªt supprimÃ©.", "success");
      qc.invalidateQueries({ queryKey: ["loans"] });
    },
  ...
  const columns = [
    { accessorKey: "loanNumber", header: "NÂ° PrÃªt" },
    {
      accessorKey: "employee",
      header: "EmployÃ©",
      cell: ({ row }: any) => {
        const e = row.original.employee;
        return e ? `${e.firstName || ""} ${e.lastName || ""}`.trim() : "â€”";
      },
    },
    {
      accessorKey: "loanType",
      header: "Type",
      cell: ({ row }: any) => row.original.loanType?.name || "â€”",
    },
    {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold">Liste des PrÃªts</h2>
          <p className="text-sm text-muted-foreground">
            GÃ©rer les demandes de prÃªts des employÃ©s
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
            <form onSubmit={formik.handleSubmit}>
              <DialogHeader>
                <DialogTitle>
                  {editing ? "Modifier le prÃªt" : "Nouvelle demande de prÃªt"}
                </DialogTitle>
              </DialogHeader>
