import { CalendarFold, ArrowDownUp, Trash2, FilePenLine } from "lucide-react";
...
interface BusWithImage extends Bus {
  image?: string;
}
...
const postData = async (id: string | number) => {
  const response = await axios.patch(`${API_ROUTE}/timetables/toggle-active/${id}`);
  return response.data;
};

const StatusData = async (id: string | number, selectedStatus: string) => {
...
export default function HoraireComponent({ items }: HoraireProps) {
  ...
  const [selectedStatus, setSelectedStatus] = useState<string | undefined>(items?.status);
  ...
  const mutationStatut = useMutation({
    mutationFn: (newStatus: string) => StatusData(items._id ?? 0, newStatus),
    onSuccess: handleSuccess,
    onError: handleError,
  });
